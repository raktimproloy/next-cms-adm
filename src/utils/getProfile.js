...
export const getProfile = async (dispatch, cookie, removeCookie) => {
  try {
    const headers = {
    ...
    // Get User Data
    const userResponse = await axios.get(`${API_HOST}user/${cookie.username}`, {
      headers: headers
    });
    const profile = userResponse.data[0];

    // Get Permission Data
    const permissionResponse = await axios.get(`${API_HOST}user-role/${cookie.permissionId}`, {
      headers: headers
    });
